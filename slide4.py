...
class Slide4(Slide):
    def __init__(self, screen, winner):
        ...
        if not winner == 0:
            self.winnercaption = Caption((240, 120), "Winner")
            self.winner = Caption((300, 300), "P" + str(winner))
        else:
            self.winnercaption = Caption((260, 120), "Draw")
            self.winner = Caption((120, 240), "Better luck next time!")
        # background
    ...
    def play(self):
        while self.running:
            ...
            if pygame.event.get(self.COLORCHANGE):
                self.winnercaption.change_color()
                self.winner.change_color()
            ...
            # redraw screen
            self.screen.blit(self.background.image, self.winnercaption.rect, self.winnercaption.rect)
            self.screen.blit(self.background.image, self.winner.rect, self.winner.rect)
            ...
            self.player1.update()
            self.player2.update()
            self.screen.blit(self.winnercaption.image, self.winnercaption.rect)
            self.screen.blit(self.winner.image, self.winner.rect)
